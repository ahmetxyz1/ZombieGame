...
// Sets default values
AFPSTemplateProjectile::AFPSTemplateProjectile()
{
...
void AFPSTemplateProjectile::BeginPlay()
{
	Super::BeginPlay();
	...
	for (AActor* Actor : Actors)
	{
		...
		{
			WindSource = Wind;
			if (UWindDirectionalSourceComponent* WindComponent = WindSource->GetComponent())
			{
				float Weight = 0.1f;
...
float AFPSTemplateProjectile::CalculateDrag()
{
	if (DragCurve)
	{	//get the information from our graph based on our projectiles velocity
		float ProjectileVelocity = ProjectileMovementComponent->Velocity.Size();
		float DragGraphValue = DragCurve->GetFloatValue(ProjectileVelocity * 0.01f);
...
// Called every frame
void AFPSTemplateProjectile::Tick(float DeltaTime)
{
	...
	if (GetWorld()->LineTraceSingleByChannel(OUT HitResult, LastPosition, GetActorLocation(), ECollisionChannel::ECC_Visibility, Params, FCollisionResponseParams::DefaultResponseParam))
